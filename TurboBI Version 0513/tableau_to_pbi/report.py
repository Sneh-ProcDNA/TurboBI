...
class ReportBuilder:
    ...
    def _expand_group_filter_members(
        ...
        print(
            f"[GROUP] Expanded filter field '{field_name}' -> '{base_field}' "
            f"members={expanded}"
        )
...
"dashboard-object"):
    ...
    def _build_sort_definition(
        ...
        for spec in sort_specs or []:
            ...
            loc = self._resolve_visual_field(
                ds_name,
                target_field,
                prefer_table=prefer_table,
                log_context="SORT",
    ...
    def _resolve_visual_field(
        ...
        *,
        ...
        print(
            f"[{log_context}] '{fname}' (ds={ds_name}) not found — "
            f"field dropped."
        )
        return None
    ...
    def _build_visual_filters(
        ...
        for idx, flt in enumerate(filters or []):
            ...
            if is_measure and members:
                print(
                    f"[FILTER] '{fname}' resolved to measure "
    ...
    def _label_object(
        ...
        if visual_type in ("tableEx", "pivotTable"):
            bag = "values"
        elif visual_type in ("map", "filledMap", "shapeMap", "azureMap"):
            bag = "dataLabels"
        else:
            bag = "labels"
