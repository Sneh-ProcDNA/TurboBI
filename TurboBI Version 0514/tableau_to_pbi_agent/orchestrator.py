...
def _remove_tree_windows_safe(path: Path, retries: int = 6) -> None:
    """Recursively remove a directory with Windows-safe retry handling.

    Handles common Windows issues:
      - delayed file handles,
      - antivirus/indexing locks,
      - read-only files,
      - partially deleted non-empty directories.
    """
    if not path.exists():
        return
    ...
    # Fallback: rename stale folder so the new run can continue.
    stale_path = path.with_name(
        f"{path.name}.__stale_delete_{int(time.time())}"
    )

    try:
        path.rename(stale_path)
        print(f"[AGENT CLEAN] Renamed locked stale output folder to: {stale_path}")

        try:
            shutil.rmtree(stale_path, onerror=_onerror)
        except OSError:
            print(
                "[AGENT CLEAN] Stale folder could not be deleted immediately. "
                "It was renamed and can be deleted manually later."
            )
...
def _clean_output_before_converter(output_dir: Path) -> None:
    """Clean generated PBIP output before invoking Converter.run()."""
    if not output_dir.exists():
        return

    # Safety guard: only auto-delete generated PBIP folders.
    if not output_dir.name.lower().endswith("_pbip"):
        print(
            f"[AGENT CLEAN] Skipping cleanup because output folder does not "
            f"end with '_pbip': {output_dir}"
        )
        return

    print(f"[AGENT CLEAN] Removing stale PBIP output before converter run: {output_dir}")
    _remove_tree_windows_safe(output_dir)
...
def _run_converter(
    ...
    # Important:
    # The agent can run the converter twice. Clean stale PBIP output here
    # so Converter.run() does not fail on Windows while removing old
    # SemanticModel/data/federated extract folders.
    _clean_output_before_converter(output_dir)
    ...
    # the agent wrapper. Tableau workbooks frequently carry arrow glyphs
    # (◀ ▶ ▲ ▼) in calc-field captions; printing those to a Windows
    # cp1252 console raises UnicodeEncodeError. Use the underlying
    # buffer with explicit UTF-8 + replace, mirroring what script.py
    # configures on direct CLI entry.
    try:
        import sys
