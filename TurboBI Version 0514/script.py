...
# Force UTF-8 on stdout / stderr so log lines containing non-ASCII glyphs
# (Tableau calc-field names commonly contain `◀ ▲ ▼ ▶` and other
# pictographic / Unicode chars) don't crash the converter when the
# console is running under cp1252 on Windows. ``errors='replace'``
# substitutes any unencodable char with `?` rather than raising
# `UnicodeEncodeError`, which is the right tradeoff for diagnostic logs
# — the actual TMDL writes already use explicit `encoding='utf-8'`.
