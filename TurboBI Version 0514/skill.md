...
Context-aware classifier (`model._build_measures` + `_formula_has_aggregation` + `_calc_field_member_filter_keys`) decides per calc field:
- **Formula contains aggregation** (SUM / AVG / COUNT / COUNTD / MIN / MAX / ATTR / TOTAL / running-* / window-* / LOOKUP / PREVIOUS_VALUE / etc.) → DAX **measure**.
- **`role='dimension'` + `tmdlType='boolean'`** → DAX **calc column** (Tableau row-level boolean predicates like `[Date] >= [Start] AND [Date] <= [End]`).
- **`role='dimension'` + no aggregation** → DAX **calc column** (IIF / IF-THEN / CASE on column refs; the dimension annotation says "axis/filter material", not "scalar context").
- **Used as a member-list filter target** (any worksheet's filter has `members=["a","b"]` on this calc) → DAX **calc column**. Without this, PBI silently drops the member filter because measures can't bind member lists.
- Otherwise → DAX **measure** (e.g. `role='measure'` + formula).

Same-table `MIN('T'[Col])` wraps emitted by the translator's scalar guard are stripped from calc-column DAX so row-context evaluation works. Cross-table parameter refs keep MIN (1-row tables make MIN return the slicer-selected value).

Logged per emission: `[CALC-COL] '<name>' on '<tbl>' emitted as calculated column (<reason>)`.
...
### CSV partition path (Hyper-extract mode)
- Emitted as a SemanticModel-relative posix path: `Source = Csv.Document(File.Contents("data/<ds_dir>/<csv>"), ...)`. PBI Desktop resolves the relative path against the SemanticModel folder, so the PBIP is portable when moved or copied.
- `safe_filename(ds_name, max_len=30)` shortens the federated-GUID directory; `safe_filename(key, max_len=40)` shortens the CSV file name. Both with deterministic 8-char hash suffix so re-runs are stable. Keeps the deepest-nested PBIP layouts under Windows MAX_PATH (260 chars).
- Fallback to absolute path only when the CSV lives outside the SemanticModel folder (rare; extract-mode emits inside by construction).
