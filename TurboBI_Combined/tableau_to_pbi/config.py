...
VISUAL_SLOTS: Dict[str, Dict[str, str]] = {
    ...
    # Buttons take no projections; the title text is the label.
    "actionButton":                   {},
}
...
# Ordinals: 0=Sum, 1=Average, 2=CountDistinct (DCount), 3=Min, 4=Max,
# 5=Count (CountNonNull), 6=Median, 7=StandardDeviation, 8=Variance.
# ---------------------------------------------------------------------------
...
AGG_TABLE: Dict[str, Dict[str, object]] = {
    ...
    "cnt":     {"label": "Count",            "fn": 5, "is_measure": True},
    "count":   {"label": "Count",            "fn": 5, "is_measure": True},
    ...
    # PBI's DCount ordinal is 2 — Count and CountDistinct are NOT the same
    # function ordinal; mapping cntd to 5 silently overcounts.
    "cntd":    {"label": "Count (Distinct)", "fn": 2, "is_measure": True},
    "ctd":     {"label": "Count (Distinct)", "fn": 2, "is_measure": True},
    ...
    # Tableau ATTR / FIRST / LAST collapse a value across a partition.
    # DAX has no direct equivalent — PBI Desktop surfaces them under
    # "First / Last" pickers on a field. Those are confusing in a
    # converted report because they don't honour Tableau's ATTR semantics
    # (return value when unique, otherwise asterisk). Mapping all three
    # to MIN (function 3) gives a deterministic scalar that matches
    # ATTR's behaviour for the typical single-value-per-group case and
    # avoids "First (Field)" / "Last (Field)" labels in the field well.
    "attr":    {"label": "Min",      "fn": 3,  "is_measure": True},
    "first":   {"label": "Min",      "fn": 3,  "is_measure": True},
    "last":    {"label": "Max",      "fn": 4,  "is_measure": True},
