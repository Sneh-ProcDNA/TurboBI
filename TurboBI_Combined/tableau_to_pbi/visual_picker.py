...
def _looks_like_button(ws: Dict[str, Any]) -> bool:
    """A button worksheet has no shelf bindings, no encodings, and
    typically a label like 'Reset Filters', 'Apply', 'Go', or 'Clear'.

    Tableau has no built-in button concept — dashboard authors fake one
    by dragging a worksheet with a single string calc field (e.g.
    ``"Reset"``) and styling the title. The converted PBI shape should
    be an `actionButton` bound to a default-state bookmark, not an
    empty scatterChart (which is what the auto-rule fallback produces
    today).

    Detection is conservative on purpose — we only flip to a button
    visual when there is literally nothing to plot AND the worksheet
    name reads like a button label.
    """
    has_any_shelf = bool(
        (ws.get("rowFields") or [])
        or (ws.get("colFields") or [])
        or (ws.get("detailFields") or [])
        or (ws.get("tooltipFields") or [])
    )
    if has_any_shelf:
        return False
    encs = [
        (ws.get("colorField") or {}).get("field"),
        (ws.get("sizeField")  or {}).get("field"),
        (ws.get("labelField") or {}).get("field"),
    ]
    if any(encs):
        return False
    label = (ws.get("name") or "").strip().lower()
    return any(tok in label for tok in (
        "reset", "clear", "apply", "go to", "navigate",
...
def pick_visual_type(ws: Dict[str, Any]) -> str:
    ...
    # 0) Button worksheets: no fields, just a label. Tableau workbooks
    #    use these for "Reset Filters" / "Apply" / nav buttons. PBI's
    #    closest match is an actionButton bound to a default bookmark.
    #    Falls through to the standard picker when the worksheet has
    #    any shelf binding (a label can still be present on a real chart).
    if _looks_like_button(ws):
        return "actionButton"
    ...
    if mapped:
        ...
        row_count = sum(1 for f in row_fields if f.get("field"))
        col_count = sum(1 for f in col_fields if f.get("field"))

        # Tabular override applied across mark mappings. A Tableau text
        # mark with fields on BOTH rows and cols is a cross-tab — that's
        # a matrix in PBI (pivotTable). The same shape on a `square`
        # heatmap is also a matrix; the rule is the shelf layout, not
        # the mark glyph.
        if mark in ("text", "square") and row_count >= 1 and col_count >= 1:
            return "pivotTable"
        if mark in ("text", "square") and (row_count + col_count) >= 2:
            return "tableEx"
        ...
        if mapped == "pieChart":
            # Tableau pie: when the worksheet has 2+ fields concentrated
            # in ONLY one shelf (rows or cols, never both), Tableau is
            # showing a donut-style breakdown — a label dim + an
            # aggregated measure ring. PBI's donutChart matches that
            # layout, while pieChart expects a single category + single
            # measure. Pies with fields on both shelves stay as pieChart
            # because that's the classic category-by-value pie.
            if (row_count >= 2 and col_count == 0) or (
                col_count >= 2 and row_count == 0
            ):
                return "donutChart"

        if mapped in ("tableEx", "pivotTable"):
            # Tableau table vs cross-tab. BOTH rows AND cols populated ⇒
            # matrix (pivotTable). One shelf only ⇒ flat table (tableEx).
...
def _shelf_facts(ws: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "row_count":       sum(1 for f in (ws.get("rowFields") or []) if f.get("field")),
        "col_count":       sum(1 for f in (ws.get("colFields") or []) if f.get("field")),
        "has_date":        has_date,
        "encoding":        encoding,
    }
    ...
    for key, expected in when.items():
        if key == "has_date":
            ...
            if not all(facts["encoding"].get(s) for s in wanted):
                return False
        elif key in ("measure_count", "dimension_count", "row_count", "col_count"):
