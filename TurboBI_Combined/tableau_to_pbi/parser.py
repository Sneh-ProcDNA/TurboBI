...
_log_calc_index = get_logger("CALC-INDEX")
...
class TWBParser:
    ...
    def _parse_datasources(self, root: ET.Element) -> None:
        for ds in root.findall("./datasources/datasource"):
            ...
            extract_filters = self._parse_extract_filters(ds, cols_map)
            datasource_filters = self._parse_datasource_filters(ds, cols_map)
            source_filters = extract_filters + datasource_filters
            ...
            self.datasources.append({
                ...
                "extracts":      extracts,
                "extractFilters": source_filters,
                "dataSourceFilters": datasource_filters,
            ...
            for view in ws_el.findall(".//view"):
                for dd in view.findall("datasource-dependencies"):
                    ...
                    for col_el in dd.findall("column"):
                        ...
                        n_added += 1
        if n_added:
            _log_calc_index.info(f"merged {n_added} worksheet-local calc "
    ...
    def _parse_connection_metadata(ds: ET.Element) -> Dict[str, Any]:
        ...
        if cls and cls.lower() not in ("federated", "hyper"):
            ...
            _log_conn.info(
                f"class='{cls}' server='{out.get('server','')}' "
    ...
    @staticmethod
    def _is_enabled_extract(extract: ET.Element) -> bool:
        enabled = (extract.get("enabled") or "true").strip().lower()
    ...
    def _parse_hyper_extracts(ds: ET.Element) -> List[str]:
        ...
        Disabled extract blocks are ignored. Tableau can leave stale
        <extract enabled='false'> metadata in a live database workbook; if
        we treat that as active extract metadata, the model layer will
        incorrectly force Power BI Import mode.

        Direct-to-hyper datasources omit the <extract> wrapper, so after
        reading enabled extract wrappers we also scan for hyper connections
        that are not inside any extract block. Multiple hyper connections
        per datasource are rare but legal; we return all of them so the
        converter can pair each one with the hyper file it actually owns.
        Paths are returned exactly as written (relative to the .twbx root)
        so the caller can match them against the file names extracted from
        the archive.
        """
        out: List[str] = []
        seen: set = set()

        def _add_dbname(conn: ET.Element) -> None:
            dbname = (conn.get("dbname") or "").strip()
            if dbname and dbname not in seen:
                seen.add(dbname)
                out.append(dbname)

        extract_conn_ids: set = set()
        for extract in ds.iter():
            if not extract.tag.endswith("extract"):
                continue
            enabled = TWBParser._is_enabled_extract(extract)
            for conn in extract.iter():
                if not conn.tag.endswith("connection"):
                    continue
                if conn.get("class", "") != "hyper":
                    continue
                extract_conn_ids.add(id(conn))
                if enabled:
                    _add_dbname(conn)
        ...
        for conn in ds.iter():
            if not conn.tag.endswith("connection"):
                continue
            if id(conn) in extract_conn_ids:
                continue
            if conn.get("class", "") != "hyper":
                continue
            _add_dbname(conn)
    ...
    def _parse_extract_filters(
        self,
        ds: ET.Element,
        cols_map: Dict[str, Tuple[str, str]],
    ) -> List[Dict[str, Any]]:
        """
        Parse Tableau datasource-level extract filters.

        Output shape consumed by model.py:

            {
                "column": "PhysicalColumnName",
                "table": "LogicalTableCaption",
                "operator": "in" | "not in" | "=" | ">=" | "between" | ...,
                "value": scalar,
                "values": [list],
                "class": "categorical" | "quantitative" | "relative-date" | ...,
                "rawColumn": original Tableau XML reference
            }

        Important:
        - Only filters under <extract> are parsed here.
        - Worksheet/view filters are already handled separately by
        ...
        def _resolve_column(raw_ref: str) -> Tuple[str, str]:
            """
            Resolve Tableau field reference to logical table + physical column
            where possible using cols_map.
            """
            ...
            candidates = [
                ...
                clean_bracket(fname),
                _strip_obj_suffix_name(clean_bracket(fname)),
            ]

            for key in candidates:
                if key in cols_map:
                    table, physical_col = cols_map[key]
                    return table, physical_col

            # Case-insensitive fallback.
            fname_norm = _strip_obj_suffix_name(fname).lower()
            for key, val in cols_map.items():
                if _strip_obj_suffix_name(key).lower() == fname_norm:
                    table, physical_col = val
                    return table, physical_col

            # Last resort: keep parsed Tableau field name.
            return "", fname
        ...
        for extract in ds.iter():
            if not extract.tag.endswith("extract"):
                continue
            if not self._is_enabled_extract(extract):
                continue

            for f in extract.iter():
                if not f.tag.endswith("filter") or f.tag.endswith("groupfilter"):
                    continue

                raw_ref = f.get("field") or f.get("column") or ""
                if not raw_ref:
                ...
                table_hint, column = _resolve_column(raw_ref)
                if not column:
                    continue

                fclass = f.get("class", "categorical")
                entry: Dict[str, Any] = {
                    "column":    column,
                    "table":     table_hint,
                    "class":     fclass,
                ...
                # Reuse existing categorical member extraction logic.
                members, exclude = self._collect_filter_members(f)

                if members:
                    entry["operator"] = "not in" if exclude else "in"
                    entry["values"] = members
                    out.append(entry)
                    continue

                # Quantitative/date/range-style extract filters.
                low = (
                    _clean_value(_first_attr(f, ("min", "from", "lower")))
                    or _clean_value(_first_attr(f, ("minimum", "lower-bound")))
                )
                high = (
                    _clean_value(_first_attr(f, ("max", "to", "upper")))
                    or _clean_value(_first_attr(f, ("maximum", "upper-bound")))
                )

                # Tableau may place range details on groupfilter children.
                for gf in f.iter("groupfilter"):
                    low = low or _clean_value(
                        _first_attr(gf, ("min", "from", "lower", "minimum", "lower-bound"))
                    )
                    high = high or _clean_value(
                        _first_attr(gf, ("max", "to", "upper", "maximum", "upper-bound"))
                    )
                    ...
                    raw_value = _clean_value(
                        _first_attr(gf, ("value", "member", "constant"))
                    )

                    if raw_op and raw_value:
                ...
                # Single-value fallback from attributes.
                value = _clean_value(
                    _first_attr(f, ("value", "member", "constant"))
                )
    ...
    def _parse_datasource_filters(
        self,
        ds: ET.Element,
        cols_map: Dict[str, Tuple[str, str]],
    ) -> List[Dict[str, Any]]:
        """Parse datasource-level filters outside active extract blocks.

        Tableau worksheet/view filters are not descendants of the top-level
        datasource node handled here. Filters that are descendants of
        <extract> are parsed by _parse_extract_filters(), so this method only
        captures datasource filters that should be applied at the Power Query
        source/partition level for both live and import modes.
        """
        extract_filter_ids: set = set()
        for extract in ds.iter():
            if not extract.tag.endswith("extract"):
                continue
            for f in extract.iter():
                if f.tag.endswith("filter"):
                    extract_filter_ids.add(id(f))

        wrapper = ET.Element("datasource")
        synthetic_extract = ET.SubElement(wrapper, "extract", {"enabled": "true"})

        for f in ds.iter():
            if not f.tag.endswith("filter") or f.tag.endswith("groupfilter"):
                continue
            if id(f) in extract_filter_ids:
                continue
            synthetic_extract.append(copy.deepcopy(f))

        return self._parse_extract_filters(wrapper, cols_map)
        ...
        for col in ds.findall("column"):
            ...
            if calc_el is not None and calc_class in {"group", "categorical-bin", "bin"}:
                ...
                if not dax_column_expr or dax_column_expr == "BLANK()":
                    ...
                    _log_group.info(
                        f"'{name}' in ds='{ds_name}': bin XML did "
    ...
    def _parse_one_worksheet(self, name: str, ws: ET.Element) -> Dict[str, Any]:
        ...
        # Structured tooltip blocks — preserves literal text spans AND
        # field placeholders with their per-run formatting. Empty list
        # when the worksheet has no ``<customized-tooltip>``. The chart
        # builder will use this to construct a PBI tooltip page; until
        # that lands, the IR carries the data so it doesn't get lost.
        custom_tooltip_blocks = self._parse_custom_tooltip_blocks(ws)
        ...
        return {
            ...
            # Structured form of the customized-tooltip block — text and
            # field-placeholder spans with per-run styling. Empty when
            # the worksheet had no <customized-tooltip>.
            "customTooltipBlocks": custom_tooltip_blocks,
            "filters":       filters,
    ...
    @classmethod
    def _parse_custom_tooltip_blocks(
        cls, ws: ET.Element,
    ) -> List[Dict[str, Any]]:
        """Return the worksheet's custom tooltip as an ordered list of
        styled blocks — preserving both literal text and field
        placeholders with their per-run formatting.

        Each block dict is one of:
            {"type": "text",  "text": "<chars>",         "style": {...}}
            {"type": "field", "field": "<name>", "agg":  "<role>",   "style": {...}}

        ``style`` may carry any of: ``bold`` (bool), ``italic`` (bool),
        ``underline`` (bool), ``fontFamily`` (str), ``fontSize`` (str
        like ``'11pt'`` or numeric), ``fontColor`` (hex string).

        Each Tableau `<run>` can mix literal text with field placeholders
        of the form ``<[ds].[role:field:k]>``. We split the run's text on
        those placeholders so a single run with mixed content produces
        multiple blocks (text + field + text...), each carrying the
        run's style. Empty text segments are skipped.

        Returns an empty list when the worksheet has no
        ``<customized-tooltip>`` element. The caller can attach the
        result to the worksheet dict and decide how to render it.
        """
        ct = ws.find(".//customized-tooltip")
        if ct is None:
            return []
        out: List[Dict[str, Any]] = []
        # Split a run's text into [text_segment | field_placeholder]
        # pieces. Tableau placeholders are ``<[ds].[role:field:k]>``;
        # the bracket-then-bracket regex captures them out.
        placeholder_re = re.compile(r"<\[[^\]]+\]\.\[([^\]]+)\]>")
        for run in ct.findall(".//run"):
            raw = run.text or ""
            if not raw:
                continue
            style = cls._parse_run_style(run)
            pos = 0
            for m in placeholder_re.finditer(raw):
                ...
                agg, fname = cls._parse_field_ref(
                    f"[ds].[{m.group(1)}]",
                )
                if fname:
                    out.append({
                ...
                pos = m.end()
            # Trailing literal text after the last placeholder (or the
            # entire run when no placeholders matched).
            if pos < len(raw):
                seg = raw[pos:]
                if seg:
                    out.append({
    ...
    def _parse_run_style(run: ET.Element) -> Dict[str, Any]:
        """Pull per-run formatting attrs into a normalised style dict.

        Tableau emits formatting on the ``<run>`` element itself —
        ``bold='true'``, ``italic='true'``, ``underline='true'``,
        ``fontname='Arial'``, ``fontsize='12'``, ``fontcolor='#757575'``.
        Boolean attrs are returned as Python booleans; the rest are
        strings. Absent attrs are simply not in the dict.
        """
        style: Dict[str, Any] = {}
        if (run.get("bold") or "").lower() == "true":
            style["bold"] = True
        if (run.get("italic") or "").lower() == "true":
            style["italic"] = True
        if (run.get("underline") or "").lower() == "true":
            style["underline"] = True
    ...
    def _parse_field_ref(cls, ref: str) -> Tuple[str, str]:
        ...
        aggregation).
        """
        def _decode_spec(spec: str) -> Tuple[str, str]:
            parts = spec.split(":")
        ...
        m = re.match(r"^\[[^\]]+\]\s*\.\s*\[(.+?)\]$", s)
        if m:
            return _decode_spec(m.group(1))
        m = re.match(r"^(\w+)\s*\(\s*\[(.+?)\]\s*\)\s*$", s)
        if m:
            return m.group(1).lower(), m.group(2).strip()
        m = re.match(r"^\[(.+)\]$", s)
        if m:
            return _decode_spec(m.group(1).strip())
        return "", s
    ...
    def _split_top(s: str, sep: str) -> List[str]:
        """Split `s` on `sep` only at the outermost level — outside both
        parentheses and Tableau field-reference brackets.

        Tableau field tokens such as `[cnt:Value-KPI/Chart:qk]` carry
        the column's literal name inside `[...]`, which can include `/`
        (the same character Tableau uses to nest dimensions on a shelf).
        Without bracket tracking the splitter cuts the token in half and
        the downstream aggregation classifier never sees the `cnt:`
        prefix, so the measure ends up on the Category axis instead of
        the Value axis. Same issue with `+` between two calc-field
        references inside a parenthesised composite expression like
        `([usr:CalcA] + [usr:CalcB])`.
        """
        out, cur = [], []
        paren = brack = 0
        for ch in s:
            if   ch == "(":             paren += 1; cur.append(ch)
            elif ch == ")":             paren -= 1; cur.append(ch)
            elif ch == "[":             brack += 1; cur.append(ch)
            elif ch == "]":             brack -= 1; cur.append(ch)
            elif ch == sep and paren == 0 and brack == 0:
    ...
    def _extract_button_style(z: ET.Element) -> Dict[str, Any]:
        ...
        if cap_font is not None:
            ...
            if "bold" in fn.lower():
                style["fontWeight"] = "bold"
            if "italic" in fn.lower():
                style["fontStyle"] = "italic"
            # Preserve the font family. Tableau encodes weight/style in
            # the fontname (e.g. 'Tableau Bold'); strip those tokens so
            # the remaining family name flows through `_safe_font_family`
            # in the report builder, which substitutes Arial for any
            # font PBI Desktop does not ship with.
            if fn:
                family = fn
                for token in ("Bold", "Italic", "Light", "Medium", "Regular",
                              "Semibold", "Semi-Bold", "Thin", "Black"):
