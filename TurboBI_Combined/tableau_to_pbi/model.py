...
from .partition_m import (
    is_databricks_live_connection,
    prefer_live_over_extract,
    render_partition_m,
    render_unsupported_partition,
)
from .utils import lineage_tag, safe_filename, tmdl_quote


# Per-tag loggers. Each ``[TAG]`` previously emitted via ``print()`` now
# routes through a named logger. Configured by ``_logging.configure_default``
# at CLI entry; callers can silence categories via:
#   logging.getLogger("tableau_to_pbi.blend").setLevel(logging.WARNING)
_log_blend     = get_logger("BLEND")
_log_blend_warn = get_logger("BLEND-WARN")
_log_calc_alias = get_logger("CALC-ALIAS")
_log_calc_col  = get_logger("CALC-COL")
_log_calc_skip = get_logger("CALC-SKIP")
_log_dax_drop  = get_logger("DAX-DROP")
_log_hyper     = get_logger("HYPER")
_log_hyper_filter = get_logger("HYPER-FILTER")
_log_meas      = get_logger("MEAS")
_log_meas_dedup = get_logger("MEAS-DEDUP")
_log_rel       = get_logger("REL")
_log_field_kind = get_logger("FIELD-KIND")
_log_group     = get_logger("GROUP")
_log_fmt_fix   = get_logger("FMT-FIX")
_log_model     = get_logger("MODEL")
_log_conn      = get_logger("CONN")
_log_repo_path = get_logger("REPO-PATH")
...
# Tableau-percent formats that need coercion-stripping when the formula
# already multiplies / divides by 100 (or 0.1 for the 0..1000 case).
# Both PBI's `0%` family and Tableau's `Percentage` macro multiply the
# stored value by 100 for display — so faithfully copying both the
# formula scaling AND the percent format double-applies the scaling.
_PERCENT_FORMATS = {"0%", "0.0%", "0.00%", "p", "percentage", "percent"}

# Detects a trailing `* 0.1`, `* 100`, or `/ 100` that's purely a
# percent-coercion factor. Allowed whitespace around the operator.
# Anchored to the end of the expression so we only strip the outermost
# factor, never an inner one inside CALCULATE() etc.
_PERCENT_COERCION_RE = re.compile(
    r"\s*\*\s*0?\.0*1\s*$"
    r"|\s*\*\s*100\s*$"
    r"|\s*/\s*100\s*$"
)
...
class SemanticModel:
    ...
    def __init__(
        ...
        # Flag flipped by `_m_file_contents_arg` when a CSV partition
        # path is rewritten to use the `RepoPath` Power Query parameter.
        # `write_tmdl` consults this to add the parameter declaration
        # to model.tmdl only when something actually uses it — keeps
        # the model file clean for live-connection workbooks.
        self._uses_repo_path: bool = False
        # Per-datasource alias map.
        self._aliases: Dict[str, Dict[str, str]] = {}
        self.tables:         List[Dict[str, Any]] = []
        self.relationships:  List[Dict[str, str]] = []
        # FieldKind registry — populated as fields are emitted, then
        # backfilled at the end of build() for native columns/measures
        # not visited by the calc-field classifier. Lets the report
        # builder do O(1) "is this a measure?" / "is this a calc column?"
        # lookups and exposes the classification reason for debugging.
        # Key: (table_name, field_name_after_rename). Value: dict with
        #   kind:   "column" | "measure"
        #   reason: short tag from _classify_calc_field, or
        #           "native column" / "native measure" for non-calc fields
        #   source: "calc" (came from <calculation>) | "native"
        self._field_kind_index: Dict[Tuple[str, str], Dict[str, str]] = {}
    ...
    def _register_group_aliases(self) -> None:
        ...
        for ds in self.datasources:
            ...
            for group_field, info in groups.items():
                ...
                if not base_field:
                    _log_group.warning(
                        f"'{group_field}' in ds='{ds_name}' has no "
                ...
                for key in group_keys:
                    ...
                    _log_group.info(
                        f"Alias fallback (no calc column emitted): "
    ...
    def _register_calc_alias_resolutions(self) -> None:
        ...
        for ds in self.datasources:
            ...
            for col in ds.get("columns") or []:
                ...
                n_resolved += 1
                _log_calc_alias.info(
                    f"'{calc_name}' (ds='{ds_name}') -> "
                    f"{loc[0]}.{loc[1]}"
                )

        if n_resolved:
            _log_calc_alias.info(f"resolved {n_resolved} trivial-alias calc "
...
# ------------------------------------------------------------------
    ...
    def build(self) -> None:
        ...
        # Backfill the FieldKind registry with native (non-calc) columns
        # and measures so model.field_kind(table, name) returns an entry
        # for every emitted field. Calc-field entries were recorded by
        # _build_measures as they were classified; this final sweep
        # picks up everything else.
        self._backfill_field_kind_index()

    def _backfill_field_kind_index(self) -> None:
        """Record every column / measure that isn't already in the index.

        The index gets calc-field entries during _build_measures (with
        the classifier's reason tag). This sweep covers everything
        else — native columns from the data source, parameter table
        columns, synthesized date-hierarchy columns, and any measure
        that was added outside the classifier path (e.g. card-value
        auto-measures from _ensure_card_value_measure).

        Entries inserted here use ``source: "native"`` and a generic
        reason so the registry remains useful for "is this a measure?"
        lookups without falsely claiming the classifier reasoned about
        the field.
        """
        for tbl in self.tables:
            ...
            for col in tbl.get("columns", []) or []:
                key = (tbl_name, col.get("name", ""))
                if not key[1] or key in self._field_kind_index:
                    continue
                # Distinguish a DAX-computed calc column (has
                # daxColumnExpr) from a plain physical / sourced column.
                if col.get("daxColumnExpr"):
                    reason = "native calc column"
                else:
                    reason = "native column"
                self._field_kind_index[key] = {
                    "kind":   "column",
                    "reason": reason,
                    "source": "native",
                }
            for meas in tbl.get("measures", []) or []:
                key = (tbl_name, meas.get("name", ""))
                if not key[1] or key in self._field_kind_index:
                    continue
                self._field_kind_index[key] = {
                    "kind":   "measure",
                    "reason": "native measure",
                    "source": "native",
                }

        # Distribution summary — one log line is plenty for sanity-checking
        # that calc fields landed where we expect. The per-field [CALC-COL]
        # / [MEAS] lines emitted during _build_measures already carry the
        # reason tag for anyone debugging an individual field. Verbose
        # per-field dumping is opt-in via FIELD_KIND_VERBOSE=1.
        import os as _os
        buckets: Dict[str, int] = {
            "native column": 0, "calc column": 0,
            "native measure": 0, "calc measure": 0,
        }
        for entry in self._field_kind_index.values():
            label = f"{entry['source']} {entry['kind']}"
            label = label.replace("calc column", "calc column").replace(
                "native column", "native column"
            )
            buckets[label] = buckets.get(label, 0) + 1
        total = sum(buckets.values())
        _log_field_kind.info(
            f"indexed {total} fields: "
            + ", ".join(f"{v} {k}" for k, v in buckets.items() if v)
        )
        if _os.environ.get("FIELD_KIND_VERBOSE") == "1":
            for (tbl, name), entry in sorted(self._field_kind_index.items()):
                _log_field_kind.info(
                    f"{tbl}.{name} -> "
                    f"{entry['kind']} ({entry['source']}: {entry['reason']})"
                )

    def field_kind(
        self, table_name: str, col_name: str,
    ) -> Optional[Dict[str, str]]:
        """Return the FieldKind entry for ``(table_name, col_name)`` or None.

        The entry is a dict with keys ``kind`` (``"column"`` or
        ``"measure"``), ``reason`` (short classifier tag for calc
        fields; generic for native fields), and ``source`` (``"calc"``
        if produced by _classify_calc_field, ``"native"`` otherwise).

        Returns None when the field is unknown — e.g. the report builder
        is mid-resolution and the field hasn't been emitted yet, or a
        caller passes a renamed-out-of-existence pair. Callers should
        treat None as "I don't know; assume column" for backward compat
        with the pre-registry behavior.
        """
        return self._field_kind_index.get((table_name, col_name))

    def is_calc_column_ref(
        self, table_name: str, col_name: str,
    ) -> bool:
        """Return True when ``(table_name, col_name)`` is a calculated
        column (has a daxColumnExpr) emitted by the classifier.

        Symmetric counterpart to ``is_measure_ref``. Lets the report
        builder distinguish a row-context calc column (which can drive
        slicers and member filters) from a plain native column (which
        can also drive those) and from a measure (which can't).
        """
        entry = self._field_kind_index.get((table_name, col_name))
        if entry is None:
            return False
        return entry["kind"] == "column" and entry["source"] == "calc"
    ...
    def _build_full(self) -> None:
        ...
        for ds in self.datasources:
            for rel in ds["relationships"]:
                ...
                if raw_key in seen_rels:
                    _log_rel.info(f"Skipping duplicate raw relationship: {ds['name']}.{rel['fromColumn']} -> {rel['toColumn']}")
                    continue
                seen_rels.add(raw_key)
                ...
                if res_key in resolved_rels:
                    _log_rel.info(f"Skipping duplicate resolved relationship: {ft}.{fc} -> {tt}.{tc}")
                    continue
        ...
        for r in self.relationships:
            ...
            if pair in active_pairs:
                r["isActive"] = False
                _log_rel.info(f"Marking inactive (ambiguous path): "
                ...
                for key_lc in sorted(shared_names):
                    ...
                    if p_dup and s_dup:
                        ...
                        if msg not in self.blend_warnings:
                            _log_blend_warn.warning(msg.replace("[BLEND-WARN] ", "", 1))
    ...
    def _deactivate_ambiguous_paths(self) -> None:
        ...
        for r in self.relationships:
            ...
            if pair in seen_pair:
                ...
                n_deactivated += 1
                if r.get("isBlend"):
                    _log_blend.info(f"deactivated parallel relationship "
            ...
            if not union(a, b):
                ...
                n_deactivated += 1
                if r.get("isBlend"):
                    _log_blend.info(f"deactivated cycle-closing relationship "
        ...
        if n_deactivated:
            _log_blend.info(f"deactivated {n_deactivated} relationship(s) to "
    ...
    #         if not ds_tables:
    #             _log_hyper.info(f"Datasource '{ds_name}' has no TMDL tables to "
    ...
    #             if not tmdl_name:
    #                 _log_hyper.info(f"No TMDL match for hyper table '{key}' "
    ...
    #                     )
    #             _log_hyper.info(f"ds='{ds_name}'  '{key}'  ->  TMDL table '{tmdl_name}'")
    ...
    def _match_hyper_tables(self) -> None:
        ...
        for ds_name, hyper_data in self.hyper_data_by_ds.items():
            ...
            if not ds_tables:
                _log_hyper.warning(
                    f"Datasource '{ds_name}' has no TMDL tables to "
                    f"enrich — hyper data ignored."
                )
                continue
            ...
            for key, meta_cols in hyper_data.metadata.items():
                ...
                if not tmdl_name:
                    _log_hyper.warning(
                        f"No TMDL match for hyper table '{key}' "
                        f"in datasource '{ds_name}' — skipped."
                    )
                    continue
    ...
    def _apply_hyper_types(
        ...
        for tbl in self.tables:
            ...
            if tbl.get("datasource") and tbl.get("datasource") != ds_name:
                _log_hyper.info(f"  Refusing to enrich '{tmdl_name}' "
            ...
            for hc in hyper_cols:
                ...
                added += 1
            _log_hyper.info(f"  {matched} columns matched, {updated} type(s) updated, {renamed} renamed, {added} added")
            break
    ...
    def _tables_for_datasource(
        ...
        # Map of parent_caption -> set(lower-cased column names) for the
        # physical columns in each group. Used below to anchor calc
        # fields whose `parentTable` wasn't set explicitly — picking the
        # group whose columns the formula actually references avoids
        # the "Sum of Patients ends up on dim_hco" bug where measures
        # default to whichever table happens to be first in iteration
        # order.
        group_cols_lc: Dict[str, set] = {}
        for parent_caption, cols in groups.items():
            group_cols_lc[parent_caption] = {
        ...
        # Pre-compute exactly ONE target parent_caption per calc field.
        # The old per-parent_caption pass appended a calc to every group
        # whose columns intersected its formula refs, so a calc like
        # ``IIF([T1.x] > 0, [T2.y], 0)`` would emit on both T1 and T2 —
        # producing a duplicate calc column or measure pair from a
        # single Tableau source. Picking one table up-front means each
        # calc field flows through ``_build_measures`` exactly once and
        # the classifier's cross-table-ref logic correctly forces the
        # measure shape for any genuine cross-group calc.
        #
        # Selection priority:
        #   1. Explicit `parentTable` if Tableau supplied one.
        #   2. Otherwise, the group with the highest overlap between
        #      the calc's formula column refs and the group's column
        #      set. Ties go to first iteration order (groups dict
        #      preserves Tableau's source ordering).
        #   3. Otherwise (constant / parameter-only calc with no
        #      resolvable refs), the first group — same fallback the
        #      old code used for the "no refs anywhere" case.
        calc_target: Dict[int, str] = {}
        for c in calc_fields:
            pt = c.get("parentTable") or ""
            if pt:
                calc_target[id(c)] = pt
                continue
            ...
            ref_names_lc = {
                n for n in ref_names_lc
                if not n.startswith("calculation_")
            }
            best_pc, best_overlap = None, 0
            for pc in groups:
                n_overlap = len(ref_names_lc & group_cols_lc.get(pc, set()))
                if n_overlap > best_overlap:
                    best_overlap = n_overlap
                    best_pc = pc
            calc_target[id(c)] = best_pc or first_group
        ...
        for parent_caption, cols in groups.items():
            ...
            table_calc_fields = [
                ...
                if calc_target.get(id(c)) == parent_caption
            ]
            out.append({
                "name":       tname,
    ...
    def _enforce_global_measure_uniqueness(self) -> None:
        ...
        for occurrences in by_name_ci.values():
            ...
            for tname, m in occurrences:
                ...
                _log_meas_dedup.info(f"Renamed measure '{old}' on table "
    ...
    def _build_measures(
        ...
        for col in calc_fields:
            ...
            # Skip calc fields the parser has already routed to the
            # DAX-calc-column emit path (Tableau groups / categorical-
            # bins). ``daxColumnExpr`` carries the synthesised
            # ``SWITCH(TRUE(), ...)`` expression; the column-render path
            # in ``_render_table_tmdl`` emits it as
            # ``column 'X' = <expr>``. Processing the same calc field
            # again here would emit a DUPLICATE — either a second calc
            # column or a same-named measure — and PBI rejects models
            # with two definitions sharing a name on one table. Groups
            # in the current corpus avoid this naturally because their
            # ``formula`` attribute is empty (Tableau stores the bucket
            # definitions in ``<groupfilter>`` children, not a formula
            # string), but a calc field that BOTH has a non-empty
            # formula AND has been compiled to a daxColumnExpr would
            # double-emit without this guard.
            if col.get("daxColumnExpr"):
                _log_calc_skip.info(
                    f"'{(col.get('caption') or col['name']).strip()}' on "
                    f"table '{table_name}' already emitted as calc column "
                    f"via daxColumnExpr (group/bin path) — skipping "
                    f"measure pass to avoid duplicate."
                )
                continue
            ...
            if (
                ...
                _log_calc_skip.info(f"blank button-label calc on table "
            ...
            if kind == "column" and dax_expr is not None:
                ...
                # Record in FieldKind registry under the final (post-
                # rename) name so the report builder's lookups match.
                self._field_kind_index[(table_name, pbi_name)] = {
                    "kind":   "column",
                    "reason": reason,
                    "source": "calc",
                }
                _log_calc_col.info(f"'{pbi_name}' on table '{table_name}' "
            ...
            if pbi_name.lower() in used_lc:
                ...
                while candidate.lower() in used_lc:
                    idx += 1
                    candidate = f"{base} {idx}"
                _log_meas.info(f"Renaming measure '{pbi_name}' -> '{candidate}' "
                      f"on table '{table_name}' (column with same name exists)")
                pbi_name = candidate
            used_lc.add(pbi_name.lower())

            # Record in FieldKind registry under the final (post-rename)
            # name so the report builder's is_measure_ref / field_kind
            # lookups match what actually got emitted.
            self._field_kind_index[(table_name, pbi_name)] = {
                "kind":   "measure",
                "reason": reason,
                "source": "calc",
            }
            _log_meas.info(f"'{pbi_name}' on table '{table_name}' "
            ...
            if dax_expr is not None:
                # Tableau-percent coercion handling.
                #
                # Tableau workbooks often write `SUM(x) * 0.1` or
                # `SUM(x) / 100` purely to massage a raw 0..100 or
                # 0..1000 measure into the 0..1 range that Tableau's
                # "Percentage" number format expects (Tableau-percent
                # multiplies the stored value by 100 for display).
                # Power BI's `0%` format string also multiplies by 100
                # for display — so faithfully copying both the formula
                # AND the percent format double-applies the scaling
                # and the user sees `950%` where Tableau showed `95%`.
                #
                # Strategy: if the source format is percent AND the
                # formula ends in a percent-coercion factor, strip the
                # factor and keep the format. The stored value stays in
                # the 0..1 range the percent format expects, matching
                # the original rendered output.
                fmt_lc = (col.get("format") or "").strip().lower()
                if fmt_lc in _PERCENT_FORMATS and _PERCENT_COERCION_RE.search(dax_expr):
                    cleaned = _PERCENT_COERCION_RE.sub("", dax_expr).rstrip()
                    _log_fmt_fix.info(
                        f"'{pbi_name}' on table '{table_name}': "
                        f"stripped Tableau percent coercion "
                        f"(kept format={col.get('format')!r})"
                    )
                    dax_expr = cleaned
                measures.append({
                    "name":       pbi_name,
                ...
                _log_dax_drop.info(f"'{pbi_name}' on table '{table_name}' "
    ...
    def _ensure_visual_fields_in_model(self) -> None:
        ...
        if added_measures or added_columns:
            _log_model.info(
                f"Added inferred visual/filter fields: "
...
# Lazy-import regex for _pick_from_candidates so the module loads
    ...
    def _add_placeholder_visual_measure(
        ...
        _log_model.info(
            f"Added placeholder measure for visual field: "
            f"ds='{ds_name}' field='{field}' -> "
    ...
    def is_measure_ref(self, table_name: str, col_name: str) -> bool:
        ...
        Backed by the FieldKind registry (O(1)). Falls back to a linear
        scan only when the registry is empty — which can happen if a
        caller invokes this before ``build()`` has populated it (e.g.
        during _ensure_visual_fields_in_model). The fallback path
        matches the historical behavior so the order-of-operations
        sensitive callers keep working unchanged.
        """
        entry = self._field_kind_index.get((table_name, col_name))
        if entry is not None:
            return entry["kind"] == "measure"
        # Fallback: registry not yet populated for this field. Linear
        # scan, identical to pre-registry behavior.
        for t in self.tables:
    ...
    def _join_cardinality(
        self,
        from_tbl: str, from_col: str,
        to_tbl:   str, to_col:   str,
    ) -> Tuple[str, str]:
        """Pick (fromCardinality, toCardinality) for a relationship.

        Default many-to-many is a footgun in PBI: it drives ambiguity
        errors, breaks Vertipaq compression, and produces fan-out on
        every aggregation. We promote to many-to-one whenever one side
        is the dimension's primary key — same shape Power BI Desktop
        picks when you draw the relationship by hand.

        Heuristic for unique-key detection (in priority order):
          1. The column carries an explicit `isPrimaryKey=True` flag
             (set by the parser when Tableau or the underlying schema
             declared it).
          2. The column name ends in 'id', '_id', 'key', or 'code' AND
             its parent table either has role='dimension' or is the
             smaller of the two tables in the join (a fact table
             rarely uses such a key as its grain).
          3. The table has fewer than 200 estimated rows and the
             column is a string with summarizeBy='none' — small lookup
             tables are unique by construction.

        Falls back to (many, many) when neither side classifies as
        unique. Better to be wrong about cardinality than to fail
        load when the user's dim table genuinely has duplicates.
        """
        def _is_unique_key(tbl: str, col: str) -> bool:
            t = next((t for t in self.tables if t.get("name") == tbl), None)
            if not t:
                return False
            for c in t.get("columns") or []:
                if c.get("name") != col:
                    continue
                # Rule 1: explicit primary-key marker.
                if c.get("isPrimaryKey"):
                    return True
                # Rule 2: dimensional-key naming convention.
                name_lc = col.lower()
                ends_with_key = (
                    name_lc.endswith("id")
                    or name_lc.endswith("_id")
                    or name_lc.endswith("key")
                    or name_lc.endswith("code")
                )
                is_dim_table = (t.get("role") or "").lower() == "dimension"
                if ends_with_key and is_dim_table:
                    return True
                # Rule 3: tiny lookup-style table.
                if (
                    c.get("summarizeBy") == "none"
                    and c.get("tmdlType") == "string"
                    and (t.get("estimatedRowCount") or 999999) < 200
                ):
                    return True
                break
            return False

        if _is_unique_key(to_tbl, to_col):
            return ("many", "one")
        if _is_unique_key(from_tbl, from_col):
            return ("one", "many")
        return ("many", "many")
    ...
    def write_tmdl(self, sem_model_dir: Path) -> None:
        ...
        # model.tmdl is emitted *after* the tables loop below — see the
        # `_write(defdir / "model.tmdl", ...)` call further down. The
        # `expression RepoPath = ...` declaration is gated on
        # `self._uses_repo_path`, which only flips to True inside
        # `_m_file_contents_arg` when a table partition's CSV path is
        # rewritten to use the parameter. Writing model.tmdl here, before
        # any table tmdl is rendered, would leave the flag False and
        # produce M code that references an undeclared RepoPath — PBI
        # Desktop then fails to load with: "The import RepoPath matches
        # no exports. Did you miss a module reference?"

        # Relationship cardinality. Tableau's noodle-style relationships
        # do not declare uniqueness on either side, so naively defaulting
        # to many-to-many is the safest *load*-time choice — but it
        # creates ambiguity errors, breaks Vertipaq compression, and
        # produces fan-out on every aggregation. When we can recognise
        # the dimension-key side (column ends in `id`/`_id`/`key`/`code`
        # on a dimension-role table, or has `isPrimaryKey=True`),
        # promote to many-to-one. Falls back to many-to-many for joins
        # we can't classify safely.
        rel_chunks: List[str] = []
        for r in self.relationships:
            ...
            # get the auto-detected shape via _join_cardinality below.
            if r.get("isBlend"):
        ...
        for t in self.tables:
            ...
            if self._credential_store is not None and t.get("connection"):
                ...
                # `render_partition_m` (in partition_m.py) picks it up —
                # overriding any Tableau-embedded customSql.
            ...
            # was extract-based.
            if (
                is_databricks_live_connection(effective_conn)
                and prefer_live_over_extract(effective_conn)
            ):
                csv_path = None
                hyper_cols = None
            ...
            if ts_customsql and cls_lc in live_classes and (
                ...
                or prefer_live_over_extract(effective_conn)
            ):
                if csv_path is not None or hyper_cols is not None:
                    _log_conn.info(
                        f"'{t['name']}' has customSql + live class "
        ...
        # Emit model.tmdl now that every table tmdl has been rendered.
        # The `_uses_repo_path` flag is final at this point, so the
        # RepoPath parameter declaration is included iff at least one
        # partition actually references it.
        ref_lines = "\n".join(
            f"ref table {tmdl_quote(t['name'])}" for t in self.tables
        )
        repo_path_expr = ""
        if getattr(self, "_uses_repo_path", False):
            # Default the parameter to the absolute path of the
            # SemanticModel folder on the build machine. The previous
            # placeholder `<<<INSTALL_PATH>>>` contained `<` and `>` —
            # illegal in Windows paths — so PBI Desktop's File.Contents
            # rejected the concatenated path with "Illegal characters
            # in path" on first load, before the user ever got a chance
            # to set the parameter via Manage Parameters.
            #
            # Using the build-machine SemanticModel path makes the .pbip
            # load and refresh successfully out of the box. When the
            # project is moved to another machine, the user updates the
            # RepoPath parameter via Model > Manage Parameters to point
            # at the new location.
            #
            # TMDL is whitespace-sensitive. The `meta [...]` record is
            # part of the M-language value of the expression; if its
            # fields are emitted on indented subsequent lines, the TMDL
            # parser interprets them as TMDL properties at the model
            # scope and fails with:
            #   UnknownKeyword: 'IsParameterQuery' is neither a
            #   property nor an object in the current context
            # Keep the meta record on the same line as the value so the
            # TMDL parser treats the whole thing as the M expression.
            # Power Query M strings do not use backslash escapes; only
            # `"` is doubled to embed a quote. Strip the Windows
            # extended-length prefix if Path.resolve() added it.
            default_repo_path = str(sem_model_dir.resolve())
            if default_repo_path.startswith("\\\\?\\"):
                default_repo_path = default_repo_path[4:]
            default_repo_path = default_repo_path.replace('"', '""')
            repo_path_expr = (
                "\n"
                f'expression RepoPath = "{default_repo_path}" '
    ...
    def _m_file_contents_arg(self, path_value: Any) -> str:
        """Return the Power Query *expression* for a File.Contents argument.

        When the path lies inside `<Name>.SemanticModel/data/...`, emit
        `RepoPath & "/data/..."` (with the RepoPath parameter resolved
        at refresh time) so the .pbip is portable across machines. When
        the path is outside the SemanticModel folder — or doesn't match
        the expected `.SemanticModel/data/...` shape — fall back to a
        quoted absolute path and log a one-line warning so the user
        knows portability is degraded.

        The boolean `self._uses_repo_path` is flipped on the first
        successful RepoPath emit; `_emit_model_tmdl_header` consults it
        to add the parameter declaration only when needed.
        """
        if isinstance(path_value, str):
            s = path_value
        else:
        ...
        if s.startswith("\\\\?\\"):
            s = s[4:]
        # Normalise separators for matching only; the emitted M code
        # uses forward slashes (Power Query accepts both, and forward
        # slashes don't need escaping).
        norm = s.replace("\\", "/")
        # Look for the `.SemanticModel/data/` anchor. Anything after
        # that is the relative-to-RepoPath fragment.
        marker = ".SemanticModel/data/"
        idx = norm.find(marker)
        if idx >= 0:
            rel = "data/" + norm[idx + len(marker):]
            rel_escaped = rel.replace('"', '""')
            self._uses_repo_path = True
            return f'RepoPath & "/{rel_escaped}"'
        # Fall back to absolute path. PBI Desktop will work, but the
        # .pbip is no longer portable.
        _log_repo_path.warning(
            f"non-portable path in M query (not under "
            f"`<name>.SemanticModel/data/`): {s}"
        )
    ...
    def _render_table_tmdl(
        ...
        if csv_path is not None:
            ...
            csv_str = self._m_file_contents_arg(csv_path)
            ...
            if m_expr is None:
                ...
                # render_partition_m returns a 2-tuple when emit succeeded.
