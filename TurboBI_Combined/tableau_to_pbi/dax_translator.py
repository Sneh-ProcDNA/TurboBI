...
# A bare numeric literal — used to short-circuit AGG(<literal>) wrappers
# such as Tableau's AVG(0) / SUM(0) layout placeholders that would
# otherwise produce invalid DAX (DAX aggregation functions reject
# scalars). Matches integers and decimals, with an optional leading sign.
...
# ---------------------------------------------------------------------------
# WINDOW_xxx -> CALCULATE + ALLSELECTED preprocessing
# ---------------------------------------------------------------------------

_WINDOW_AGG_RE = re.compile(
    r"WINDOW_(SUM|AVG|MIN|MAX|COUNT)\s*\(",
    re.IGNORECASE,
)

# Map Tableau WINDOW_xxx to the matching DAX scalar aggregation. Both
# the iterator (-X) and the non-iterator forms are listed; we use the
# scalar form because the inner expression that WINDOW wraps is already
# scalar (e.g. WINDOW_SUM(MAX([Value]))).
_WINDOW_TO_DAX = {
    "SUM":   "SUM",
    "AVG":   "AVERAGE",
    "MIN":   "MIN",
...
def _translate_window(
    ...
    window_subs: Optional[Dict[str, str]] = None,
) -> Optional[str]:
    """Preprocess WINDOW_SUM / AVG / MIN / MAX / COUNT into a
    CALCULATE + ALLSELECTED shape.

    Tableau's WINDOW_xxx evaluates an aggregate across the visual's
    partition — i.e. all rows currently in the same row/cell context
    as the calling row, ignoring filters that come from the iterator
    row. The DAX analogue is

        CALCULATE(<scalar_agg>(<inner>), ALLSELECTED('<table>'))

    which preserves the visual's overall filter context but removes
    the current-row restriction. That's the right shape for the
    common ``MAX([Value]) / WINDOW_SUM(MAX([Value]))`` percent-of-total
    pattern.

    Substitutes a unique placeholder for each WINDOW call and stores
    the DAX replacement in `window_subs` (the caller restores the
    placeholders AFTER the main translator has run, so the placeholder
    identifier flows through tokenisation unchanged).

    Returns the formula with placeholders, or None if any inner
    expression couldn't be translated (caller then falls back to the
    original formula and lets the unsupported-token check reject it).
    """
    if window_subs is None:
        window_subs = {}
    out_parts: List[str] = []
    pos = 0
    for m in _WINDOW_AGG_RE.finditer(formula):
        fn = m.group(1).upper()
        # Walk forward from the opening paren to find its match,
        ...
        while i < len(formula) and depth:
            ...
            i += 1
        if depth:                              # unbalanced parens
            return None
        inner_src = formula[m.end(): i - 1]   # strip outer parens
        # Tokenise & translate the inner expression with the main
        # translator. Empty / unparseable inner -> bail.
        inner_tokens = _tokenize(inner_src)
        if not inner_tokens:
            return None
        inner_dax = _translate_tokens(
            inner_tokens, 0, len(inner_tokens),
            table_name, col_aliases, datasource_columns,
            field_to_pbi, parameter_refs,
        )
        if inner_dax is None:
            return None
        scalar_agg = _WINDOW_TO_DAX[fn]
        # If the inner DAX is itself a single-level aggregation around a
        # bare column reference — e.g. ``MAX('T'[X])`` — wrapping it in
        # another scalar aggregation yields ``SUM(MAX('T'[X]))``, which
        # DAX rejects at query time (SUM requires a column, MAX returns
        # a scalar). The common Tableau form ``WINDOW_SUM(MAX([X]))``
        # semantically means "partition sum of X" — inner MAX in a
        # per-row context degenerates to the row value, then WINDOW_SUM
        # sums across the partition. Unwrap to
        # ``CALCULATE(SUM('T'[X]), ALLSELECTED('T'))`` which is the
        # correct DAX and the form Power BI users hand-write for this
        # pattern. Falls back to the wrapped form when the inner is a
        # composite expression we can't safely unwrap.
        inner_agg_re = re.compile(
            r"^\s*(MAX|MIN|SUM|AVERAGE|COUNT|COUNTA|COUNTROWS)\s*\(\s*"
            r"('[^']+'\[[^\]]+\])\s*\)\s*$",
            re.IGNORECASE,
        )
        m_inner = inner_agg_re.match(inner_dax)
        if m_inner:
            col_ref = m_inner.group(2)
            dax_repl = (
                f"CALCULATE({scalar_agg}({col_ref}), "
                f"ALLSELECTED('{table_name}'))"
            )
        else:
            # Conservative form — invalid DAX at query time but model
            # still loads. Preferable to dropping the measure entirely;
            # users can hand-edit in the model view.
            dax_repl = (
                f"CALCULATE({scalar_agg}({inner_dax}), "
                f"ALLSELECTED('{table_name}'))"
            )
        ph = f"__WINDOW_PLACEHOLDER_{len(window_subs)}__"
        window_subs[ph] = dax_repl
        out_parts.append(formula[pos: m.start()])
        out_parts.append(ph)
        pos = i
    out_parts.append(formula[pos:])
...
def translate_tableau_to_dax(
    ...
    # WINDOW_xxx preprocessing — same placeholder shuttle as LOD FIXED.
    # Runs AFTER LOD so a WINDOW wrapped around a FIXED block can pick
    # up the LOD's placeholder identifier rather than its raw braces.
    window_subs: Dict[str, str] = {}
    f_win = _translate_window(
        f, table_name, col_aliases, datasource_columns,
        field_to_pbi, parameter_refs, measure_refs,
        window_subs,
    )
    if f_win is not None:
        f = f_win
    # else: WINDOW preprocess failed — fall through; the formula either
    # had no WINDOW calls (so this is a no-op) or one of them couldn't
    # be translated. The unsupported-token check below will then catch
    # any remaining WINDOW_xxx and reject the formula cleanly.

    _UNSUPPORTED_TOKENS = {
        # LOD expressions we don't translate yet (FIXED is handled above)
        "include", "exclude",
        # Table calculations we still don't have a clean DAX shape for.
        # WINDOW_SUM/AVG/MIN/MAX/COUNT are handled by _translate_window;
        # RANK / RANK_UNIQUE / RANK_DENSE are handled by the token
        # translator's RANK family branch. The remaining window aggs
        # (stdev/var) and ranking helpers stay rejected.
    ...
    try:
        ...
        if result is not None:
            ...
            if window_subs:
                # Same shuttle for WINDOW placeholders.
                for ph, dax in window_subs.items():
                    result = result.replace(ph, dax)
            if rowcount_subs:
...
def _translate_tokens(
    ...
    while i < end:
        ...
        if low in _AGG_MAP and i + 1 < end and tokens[i + 1] == "(":
            ...
            elif _NUMERIC_LITERAL_RE.fullmatch(stripped):
                # AGG(<numeric literal>) -- Tableau's layout placeholder
                # pattern (AVG(0), SUM(0), MIN(0)). DAX aggregation
                # functions reject scalars, so wrapping `0` in AVERAGE()
                # produces invalid DAX that breaks model load. Drop the
                # wrapper and emit the literal directly — the visual
                # binds to the scalar value, preserving Tableau's
                # original intent (the placeholder reserved a slot at
                # value `0`).
                dax_tokens.append(stripped)
            else:
        ...
        # -- Tableau RANK family -> DAX RANKX(ALLSELECTED(...))  ----------
        # RANK_UNIQUE(expr [, 'asc'|'desc'])  -> RANKX(..., expr, , dir, Skip)
        # RANK_DENSE(expr  [, 'asc'|'desc'])  -> RANKX(..., expr, , dir, Dense)
        # RANK(expr        [, 'asc'|'desc'])  -> RANKX(..., expr, , dir, Skip)
        # Direction defaults to ``desc`` in Tableau. The partition Tableau
        # uses is the visual's pane; ``ALLSELECTED('<table>')`` is the
        # closest DAX equivalent that respects the visual's other filters
        # while still ranking across the full visible set.
        if (low in ("rank", "rank_unique", "rank_dense")
            ...
            if not parts:
                return None
            expr = _translate_token_list(
                parts[0], table_name, col_aliases, datasource_columns,
                field_to_pbi, parameter_refs,
            )
            if expr is None:
                return None
            direction = "DESC"
            if len(parts) >= 2:
                dir_arg = _join_tokens(parts[1]).strip()
                if dir_arg.startswith(("'", '"')) and dir_arg.endswith(("'", '"')):
                    dir_arg = dir_arg[1:-1]
                if dir_arg.lower() == "asc":
                    direction = "ASC"
            ties = "Dense" if low == "rank_dense" else "Skip"
            dax_tokens.append(
                f"RANKX(ALLSELECTED('{table_name}'), {expr}, , "
                f"{direction}, {ties})"
            )
            i = j + 1
            continue
        ...
        # -- WINDOW-aggregation placeholder -------------------------------
        # ``__WINDOW_PLACEHOLDER_N__`` markers are emitted by
        # ``_translate_window`` when it lifts a ``WINDOW_SUM(...)`` /
        # ``WINDOW_AVG(...)`` etc. expression. Same pass-through pattern
        # as LOD blocks — the CALCULATE/ALLSELECTED replacement happens
        # in ``translate_tableau_to_dax`` after the main translator
        # finishes.
        if re.fullmatch(r"__WINDOW_PLACEHOLDER_\d+__", tok):
            dax_tokens.append(tok)
            i += 1
            continue
...
def _translate_block_case(
    ...
    # Scan WHEN / THEN / ELSE at depth 0 only. A nested block-style
    # CASE or IF inside a WHEN's THEN branch would otherwise contribute
    # its own keywords to these lists and corrupt the parse.
    when_positions = _depth_zero_positions(body, {"when"})
    if not when_positions:
        return None
    then_positions = _depth_zero_positions(body, {"then"})
    else_positions = _depth_zero_positions(body, {"else"})
    ...
    for k, when_pos in enumerate(when_positions):
        # First depth-0 THEN strictly after this WHEN.
        then_pos = next((j for j in then_positions if j > when_pos), None)
        if then_pos is None:
            return None
        # Section runs from THEN+1 to the next depth-0 section delimiter
        # (next WHEN, or the final ELSE, or end of body).
        next_pos = len(body)
        if k + 1 < len(when_positions):
            next_pos = when_positions[k + 1]
        else:
            tail_else = next((j for j in else_positions if j > then_pos), None)
    ...
    tail_else = next((j for j in else_positions if j > last_when), None)
    if tail_else is not None:
...
def _translate_block_if(
    ...
    # Scan section delimiters at depth 0 only. A nested block-style
    # CASE or IF in the THEN branch would otherwise contribute its own
    # THEN/ELSE/ELSEIF keywords and corrupt the parse.
    delimiters = _depth_zero_positions(body, {"then", "elseif", "else"})
    if not delimiters or body[delimiters[0]].lower() != "then":
        return None

    # Walk the delimiter list as a (THEN [, ELSEIF / ELSE])* sequence.
    # Each THEN closes a condition; the following section runs to the
    # next delimiter (or end of body). ELSEIF/ELSE delimit the previous
    # result section.
    args: List[str] = ["TRUE()"]
    cond_start = 0
    cond_end: Optional[int] = None
    result_start: Optional[int] = None

    def _emit(cond_lo, cond_hi, res_lo, res_hi):
        cond = _translate_token_list(
            body[cond_lo: cond_hi],
            table_name, col_aliases, datasource_columns, field_to_pbi, parameter_refs,
        )
    ...
    for di, pos in enumerate(delimiters):
        low = body[pos].lower()
        if low == "then":
            cond_end = pos
            result_start = pos + 1
        ...
        if low in ("elseif", "else"):
            if cond_end is None or result_start is None:
                return None
            if not _emit(cond_start, cond_end, result_start, pos):
                return None
            if low == "elseif":
                cond_start = pos + 1
                cond_end = None
    ...
    # Reached end of delimiter list without ELSE — emit the trailing
    # result section that runs from result_start to end of body.
    if cond_end is None or result_start is None:
        return None
    if not _emit(cond_start, cond_end, result_start, len(body)):
        return None
    return (f"SWITCH({', '.join(args)})", end_pos + 1)
...
def _depth_zero_positions(
    body: List[str], targets: set,
) -> List[int]:
    """Indices of tokens in ``body`` whose lowercase form is in
    ``targets``, seen at depth 0 (i.e. NOT inside a nested
    block-style CASE/IF). Used by ``_translate_block_if`` and
    ``_translate_block_case`` to scan their section delimiters
    (THEN / ELSEIF / ELSE / WHEN) without mistakenly matching the
    same keywords from inside a nested block.

    Bare ``CASE`` / ``IF`` (NOT followed by ``(`` — function-call form
    is a different construct) opens a block; ``END`` closes one.
    """
    out: List[int] = []
    depth = 0
    n = len(body)
    ...
    while j < n:
        low = body[j].lower()
        if (low in ("case", "if")
                and (j + 1 >= n or body[j + 1] != "(")):
            depth += 1
        elif low == "end":
            depth -= 1
        elif depth == 0 and low in targets:
            out.append(j)
        j += 1
    return out
...
_FN_PASSTHROUGH: Dict[str, str] = {
    ...
    "isnull":  "ISBLANK",
    # Zero-arg date functions. TODAY / NOW appear in Tableau formulas
    # as ``TODAY()`` / ``NOW()`` — the parens come through as separate
    # ``(`` / ``)`` tokens. Putting these in the passthrough table (not
    # ``_KW_LITERAL``) lets the natural identifier-followed-by-paren
    # join collapse them to ``TODAY()`` / ``NOW()``. Listing the parens
    # in the mapped value would emit them twice (``TODAY() ()``).
    "today":   "TODAY",
    "now":     "NOW",
...
def _join_tokens(tokens: List[str]) -> str:
    ...
    for i, t in enumerate(out_tokens):
        ...
        prev = out_tokens[i - 1]
        # No space:
        #   - after an opening paren / before a closing paren
        #   - around commas
        #   - between an identifier and an opening paren (function call):
        #     prevents `USERNAME ()` / `YEAR ([Date])` from leaking out.
        prev_is_ident = bool(prev) and (prev[-1].isalnum() or prev[-1] == "_")
        if prev == "(" or t == ")" or t == "," or prev == ",":
            out += t
        elif t == "(" and prev_is_ident:
            out += t
        else:
            out += " " + t
    return out.strip()
