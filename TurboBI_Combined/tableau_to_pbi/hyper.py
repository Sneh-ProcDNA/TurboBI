...
_log_hyper_match = get_logger("HYPER-MATCH")
...
def load_hyper(hyper_path: Path) -> HyperData:
    ...
    if not _HYPER_OK:
        _log_hyper.warning("pantab / tableauhyperapi not installed — hyper data skipped.")
        return hd
    ...
    try:
        ...
        _log_hyper.info(f"Could not read catalog from {hyper_path.name}: {exc}")
        return hd
    ...
    try:
        ...
        for tname, df in raw.items():
            ...
            hd.frames[key] = df
        _log_hyper.info(f"Loaded {len(hd.frames)} table(s) from {hyper_path.name}")
    except Exception as exc:
        _log_hyper.info(f"Could not load frames from {hyper_path.name}: {exc}")
    ...
    def bind(
        ...
        for ds in datasources:
            for dbname in ds.get("extracts", []) or []:
                ...
                if abs_path is None:
                    _log_hyper.info(f"Datasource '{ds['name']}' references "
                ...
                _log_hyper.info(f"Datasource '{ds['name']}' (caption "
        ...
        if not out and self._paths_in and datasources:
            ...
            if hd is not None:
                ...
                _log_hyper.info(f"No <extract> mapping found; falling back "
...
def write_csv(hd: HyperData, data_dir: Path) -> Dict[str, Path]:
    ...
    """
    import os
    # Same `\\?\` long-path treatment as below: `mkdir(parents=True)`
    # walks the path one segment at a time and the intermediate
    # segments can already exceed MAX_PATH even before we start adding
    # the CSV filename.
    if os.name == "nt":
        abs_dir = str(data_dir.resolve())
        long_dir = abs_dir if abs_dir.startswith("\\\\?\\") else "\\\\?\\" + abs_dir
    ...
    import os
    # On Windows, `Path.open()` and `df.to_csv()` hit the 260-char
    # MAX_PATH ceiling whenever the user's output root + the workbook's
    # `<name>.SemanticModel/data/<ds>/<csv>` tail crosses that. The
    # `\\?\` prefix opts the path into the long-path Win32 API
    # (32 767-char limit), which pandas / Python's CSV writer both
    # honour. Non-Windows OSes get the bare path; on Windows we feed
    # `df.to_csv` the prefixed form but keep `paths[key]` as the plain
    # Path so downstream code (write_tmdl's relative-path resolution)
    # works against the regular filesystem view.
    ...
    for key, df in hd.frames.items():
        ...
        # Use a tighter 40-char cap here so the file name plus the
        # ~33-char federated.GUID parent dir plus the ~50-char
        # ``<name>.SemanticModel/data/`` prefix leaves ~130 chars of
        # headroom for the user's chosen output root — enough for
        # PBIPs nested 4 directories deep into a normal home folder.
        # The `\\?\` long-path opt-in below is the safety net for the
        # cases where workbook names alone push us over the limit.
        safe_key = safe_filename(key, max_len=40)
        csv_path = (data_dir / f"{safe_key}.csv").resolve()
        df.to_csv(_long_path(csv_path), index=False, encoding="utf-8-sig")
        paths[key] = csv_path
        _log_hyper.info(f"{len(df):,} rows  ->  {csv_path.name}")
...
# def match_extract_filters_to_hyper_table(
#     hyper_key: str,
#     hyper_cols: List[str],
#     extract_filters: List[Dict[str, Any]],
# ) -> List[Dict[str, Any]]:
#     """Return the subset of extract filters that plausibly refer to the hyper table.

#     This is a simple heuristic filter: we keep extract filters that reference
#     any of the hyper columns, since those are more likely to be relevant to
#     the data in that table. It's not perfect but helps reduce noise when a
#     datasource has many extract filters but only some of them apply to the
#     hyper table we're examining.
#     """
#     hyper_col_set = {c.lower() for c in hyper_cols}
#     relevant_filters = []
#     for ef in extract_filters:
#         # Check if any column in the filter matches a hyper column (case-insensitive)
#         filter_cols = ef.get("columns", [])
#         if any(fc.lower() in hyper_col_set for fc in filter_cols):
#             relevant_filters.append(ef)
#     return relevant_filters

def _strip_tableau_brackets(name: str) -> str:
    """Normalize Tableau field references like [Region] or [Table].[Region]."""
    if not name:
        return ""
...
def _filter_column_name(flt: Dict[str, Any]) -> str:
    """
    Return the column name from a tolerant extract-filter dictionary.

    Supports multiple possible parser/output shapes:
      - column
      - field
      - name
      - sourceColumn
      - sourceCol
    """
    for key in ("column", "field", "name", "sourceColumn", "sourceCol"):
        if flt.get(key):
            return _strip_tableau_brackets(str(flt[key]))

    return ""


def _filter_table_hint(flt: Dict[str, Any]) -> str:
    """
    Return optional table/hyper-table hint from the extract-filter dictionary.

    The parser should ideally populate "table" using colsMap resolution.
    Other keys are supported as fallback shapes.
    """
    for key in ("table", "parentTable", "hyperKey", "relation", "sourceTable"):
        if flt.get(key):
            return str(flt[key])
...
def match_extract_filters_to_hyper_table(
    ...
    """
    Return extract filters that should apply to this Hyper table.

    Matching strategy:
      1. If the filter has a table hint, it must loosely match the Hyper
         table key or the TMDL table matched to that Hyper table.
      2. The filter column must physically exist in the Hyper table.
      3. Column matching supports exact names and normalized Tableau/Hyper names.

    Args:
        hyper_key:
            Hyper table key, usually in the format "Schema.Table".

        hyper_cols:
            Physical column names present in the Hyper table.

        extract_filters:
            Parsed datasource-level extract filters from parser.py.

    Returns:
        A list of filter dictionaries relevant to this Hyper table.

        Each returned dictionary preserves the original filter payload, but
        normalizes/overwrites "column" with the exact Hyper column name so
        model.py can safely emit Power Query M against the CSV headers.
    """
    if not extract_filters:
        return []

    hyper_key_norm = _normalize_name(hyper_key)
    tmdl_table_norm = _normalize_name(tmdl_table_name)
    ...
    # Fuzzy normalized column lookup.
    # Example:
    #   "Order_Date"  -> "order date"
    #   "Order Date"  -> "order date"
    hyper_col_norm_lookup = {
        _normalize_name(c): c
        for c in hyper_cols
        if c
    }
    ...
    for flt in extract_filters:
        if not isinstance(flt, dict):
            continue

        col = _filter_column_name(flt)
        if not col:
            continue

        # If the parser supplied a table hint, use it to avoid applying
        # a filter to the wrong logical table when multiple Hyper tables
        # have the same column name.
        table_hint = _filter_table_hint(flt)
        if table_hint:
            hint_norm = _normalize_name(table_hint)

            hyper_match = (
                hint_norm in hyper_key_norm
                or hyper_key_norm in hint_norm
            )
            tmdl_match = bool(tmdl_table_norm) and (
                hint_norm in tmdl_table_norm
                or tmdl_table_norm in hint_norm
            )
            if not hyper_match and not tmdl_match:
                continue

        resolved_col = (
            hyper_col_lookup.get(col.lower())
            or hyper_col_norm_lookup.get(_normalize_name(col))
        )

        if not resolved_col:
            continue

        # Preserve original filter attributes, but replace "column" with
        # the exact Hyper/CSV column name.
        f2 = dict(flt)
        f2["column"] = resolved_col
        matched.append(f2)
...
def match_hyper_to_tmdl(
    ...
    _log_hyper_match.info(f"'{hyper_key}' (norm='{hyper_tname_low}') -> "
