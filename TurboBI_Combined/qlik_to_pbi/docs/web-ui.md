...
> **Progress stages (2026-06 fix, combined app `templates/qlik.html` + `templates/tableau.html`):** the step list classifies each streamed log line with a regex per stage, scanning FORWARD from the current step (monotonic — a line can advance, never regress). The regexes are anchored to the converter's module log prefixes (`[UNBUILD]`/`[TEXTEVAL]`, `Engine extract`/`hypercube:`/`-> *.parquet (`, `[1-2/4]`/`[MODEL]`, `[3/4]`/`[REPORT]`, `[4/4]`/`[WRITE]`/`[UI] Zipping`), NOT generic keywords — words like "written"/"output"/"done" occur in every stage's lines (the fetch logs "5/5 table(s) written") and used to mis-jump the bar to "Packaging .pbip / 80%" mid-extract. Progress is weighted (fetch owns 12%→62% of the bar) with intra-fetch advancement from per-table completion lines counted against the "across N pool table(s)" plan; sliced tables log one line per slice + a merge line, so the count is clamped (`Math.min`) — the bar may saturate the fetch span early but never misstates the stage.
