...
- [visual-and-emit-details.md](visual-and-emit-details.md) — visual styling pipeline (title in `visualContainerObjects`), expression-dim calc columns, bar/combo subtype matrix, pie↔donut, slicer dropdown default, axis show/title from Qlik, KPI force_measure, map field-wells, sn-nav-menu, cardVisual chrome, azureMap viewport, sheet title resolution, script-derived renames/partitions, What-If params, visual_rules.json, engine-schema sidecar, engine field tags
