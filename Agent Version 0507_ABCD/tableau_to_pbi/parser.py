...
class TWBParser:
    ...
    def _parse_datasources(self, root: ET.Element) -> None:
        for ds in root.findall("./datasources/datasource"):
            ...
            columns, col_aliases = self._parse_columns(ds, name, caption,
                                                        col_parent, objects,
                                                        cols_map)
    ...
    @staticmethod
    def _compile_categorical_bin(calc_el: ET.Element) -> str:
    ...
    def _parse_columns(
        ...
        for col in ds.findall("column"):
            ...
            # Categorical-bin calcs (e.g. <calculation class='categorical-bin'
            # column='[Payer type]'>) have NO formula attribute — the bin
            # definition lives in nested <bin>/<value> children. Without
            # explicit handling, these landed in `_build_columns` with
            # `sourceColumn: <bin name>` pointing at a header that doesn't
            # exist in the underlying CSV → visuals show "Fix Now" because
            # the field can't bind to data. Synthesize a DAX expression
            # from the bin definitions and stash it as `daxColumnExpr` so
            # the model layer emits a TMDL calculated column instead.
            dax_column_expr = ""
            if calc_el is not None and not formula:
                cls = (calc_el.get("class") or "").strip()
                if cls == "categorical-bin":
                    dax_column_expr = TWBParser._compile_categorical_bin(calc_el)

            canonical = _strip_obj_suffix_name(name)
            parent    = parent_for(name, formula)
